...
class GereRevisoes {
  ...
  /**
   * Atualiza as observações de uma revisão na base de dados.
   *
   * @param st              Uma instância de Statement para executar queries na base de dados.
   * @param aIdRevisao      Id do pedido de revisão.
   * @param aObservacao    Nova observação associada ao pedido de revisão.
   * @return true se a operação for bem-sucedida, false caso contrário.
   */
  public boolean adicionarObservacao(Statement st, int aIdRevisao, String aObservacao) {
      try {
          String query = "UPDATE revisao SET observacoes = '" + aObservacao + "' WHERE idrevisao = " + aIdRevisao;
