/** Programa para gerir utilizadores
...
class GereUtilizadores {
  ...
  //Atualiza Username na base de dados dos autores
  public boolean atualizaUsernameAutores(Statement st, int aIdUtilizador, String aNovoUsername) {
    try {
      st.executeUpdate("UPDATE utilizadores SET login = '" + aNovoUsername+"' WHERE idutilizador = " + aIdUtilizador);
  ...
  //Atualiza NIF na base de dados dos autores
  public boolean atualizaNIFAutores(Statement st, int aIdUtilizador, int aNovoNIF){
    try {
      // Chamar a função verificaFormatoNIF(int aNIF) para ver se o NIF está no formato correto
      if (verificaFormatoNIF(aNovoNIF)){
        //Usar a função verificaNIF(Statement st, int aNIF) para ver se o NIF existe na base de dados
        if (verificaNIF(st, aNovoNIF) == false){
          st.executeUpdate("UPDATE autores SET nif = '" + aNovoNIF+"' WHERE utilizadores_idutilizador = " + aIdUtilizador);
          return true;
        }
        return false;
      }
